...
public class StoreOwnerTest extends ProjectTest{
    ...
    //TODO: After Tear Down clear the data from the system : initialize again system
    ...
    public void testGetManagerPermissionInStoreAfterChange(){
        ...
        assertNotNull(permissions);
        assertTrue(permissions.size() > 0);
        status = this.changeStoreManagerPermissions(uid, storeId, permissions2change);
        assertTrue(status > 0);
        permissions = this.getManagerPermissionInStore(uid, storeId, uIdManager);
        assertNotNull(permissions);
        assertTrue(permissions.size() > 0);
    }
    ...
    public void testAfterPurchaseGetStorePurchasesHistory(){
        ...
        purchases = this.getStorePurchasesHistory(uid, storeId);
        assertNotNull(purchases);
        assertTrue(purchases.size() > 0);
    }
    ...
    public void testChangeStoreManagerPermissions() {
        ...
        permissions.add(1);
        permissions.add(2);
        status = this.changeStoreManagerPermissions(uid, uIdManager, permissions);
        assertTrue(status > 0);
    }
    ...
    public void testChangeStoreManagerPermissionsOnSomeoneElse() {
        ...
        permissions.add(1);
        permissions.add(2);
        int status = this.changeStoreManagerPermissions(uid, uIdManager, permissions);
        assertTrue(status < 0);
    }
    ...
    public void testChangeStoreManagerPermissionsOnOwner() {
        ...
        permissions.add(1);
        permissions.add(2);
        status = this.changeStoreManagerPermissions(uid, uIdOwner, permissions);
        assertTrue(status < 0);
    }
    ...
    public void testNotOwnerTryChangeStoreManagerPermissions() {
        ...
        permissions.add(1);
        permissions.add(2);
        status = this.changeStoreManagerPermissions(notOwnerId, uIdManager, permissions);
        assertTrue(status < 0);
    }
    ...
    public void testAppointmentUnRealManagerInStore() {
        int uid = this.users_dict.get(users[0][USER_EMAIL]).getUserId();
        int storeId = stores.get(0).getStoreId();
        int status = this.appointmentMangerInStore(uid, storeId,ERROR);
        assertTrue(status < 0);
    }
    ...
    public void testAppointmentExistOwnerToMangerInStore() {
        ...
        int status = this.appointmentOwnerInStore(uid, storeId,uIdOwner);
        assertTrue(status > 0);
        status = this.appointmentMangerInStore(uid, storeId,uIdOwner);
        assertTrue(status < 0);
    }
    ...
    public void testAppointmentExistManagerInStore() {
        ...
        int status = this.appointmentMangerInStore(uid, storeId,uIdManager);
        assertTrue(status > 0);
        status = this.appointmentMangerInStore(uid, storeId,uIdManager);
        assertTrue(status < 0);
    }

    @Test
    public void testNotOwnerTryAppointmentManagerInStore() {
        int uid = this.users_dict.get(users[2][USER_EMAIL]).getUserId();
        int storeId = stores.get(0).getStoreId();
        int uIdManager = this.users_dict.get(users[1][USER_EMAIL]).getUserId();
        int status = this.appointmentMangerInStore(uid, storeId,uIdManager);
        assertTrue(status < 0);
    ...
    public void testAppointmentExistMangerToOwnerInStore() {
        ...
        int status = this.appointmentMangerInStore(uid, storeId,uIdManager);
        assertTrue(status > 0);
        status = this.appointmentOwnerInStore(uid, storeId,uIdManager);
        assertTrue(status > 0);
    }
    ...
    public void testAppointmentExistOwnerInStore() {
        ...
        int status = this.appointmentOwnerInStore(uid, storeId,uIdOwner);
        assertTrue(status > 0);
        status = this.appointmentOwnerInStore(uid, storeId,uIdOwner);
        assertTrue(status < 0);
    }
    ...
    public void testAppointmentUnRealOwnerInStore() {
        int uid = this.users_dict.get(users[0][USER_EMAIL]).getUserId();
        int storeId = stores.get(0).getStoreId();
        int status = this.appointmentOwnerInStore(uid, storeId,ERROR);
        assertTrue(status < 0);
    }
    ...
    public void testAddExistsProduct() {
        ...
        int uid = this.users_dict.get(users[0][USER_EMAIL]).getUserId();
        ProductInfo product2Add = goodProduct0;
        productId0 = this.addProduct(uid, stores.get(1).getStoreId(), product2Add.getCategories(), product2Add.getName(), product2Add.getDescription()
                , product2Add.getPrice(), product2Add.getQuantity());
        assertTrue(productId0 > 0);
        productId1 = this.addProduct(uid, stores.get(1).getStoreId(), product2Add.getCategories(), product2Add.getName(), product2Add.getDescription()
                , product2Add.getPrice(), product2Add.getQuantity());
        assertTrue(productId1 < 0);
    }
    ...
    public void testRemoveProduct() {
        ...
        int uid = this.users_dict.get(users[0][USER_EMAIL]).getUserId();
        ProductInfo product2Add = goodProduct0;
        productId0 = this.addProduct(uid, stores.get(2).getStoreId(), product2Add.getCategories(), product2Add.getName(), product2Add.getDescription()
                , product2Add.getPrice(), product2Add.getQuantity());
        assertTrue(productId0 > 0);
        status = this.removeProduct(uid, stores.get(2).getStoreId(), productId0);
        assertTrue(status > 0);
    }
    ...
    public void testNotOwnerTryRemoveProduct() {
        ...
        productId0 = this.addProduct(uid, stores.get(2).getStoreId(), product2Add.getCategories(), product2Add.getName(), product2Add.getDescription()
                , product2Add.getPrice(), product2Add.getQuantity());
        assertTrue(productId0 > 0);
        uid = this.users_dict.get(users[1][USER_EMAIL]).getUserId();
        status = this.removeProduct(uid, stores.get(2).getStoreId(), productId0);
        assertTrue(status < 0);
    }
