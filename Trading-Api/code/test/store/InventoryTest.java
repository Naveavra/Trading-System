...
public class InventoryTest {
    ...
    void testAddProduct_ValidInput() throws Exception{
        ...
        inventory.addProduct("OG Kush","A familiar brand with a fruity scent.",ids,10, 5, categories, null);
        Product p = inventory.getProduct(ids.get()-1);
        assertNotNull(p);
        assertEquals(p.getName(),"OG Kush");
        assertEquals(0,p.getID());
        inventory.addProduct("Lemon-Haze","Lemony flavors laced in the sweat of your enemies.",ids,10, 5, categories, null);
    ...
    void testAddProduct_InvalidInput() throws Exception{
        List<String> categories = new ArrayList<>();
        inventory.addProduct("Waffles","Fluffy, tasty and makes you fat AF.",ids,10, 5, categories, null);
        assertThrows(Exception.class, ()->inventory.addProduct("Waffles","Fluffy, tasty and makes you fat AF.",ids,10, 5, categories, null));
    ...
    void testGetProductByName_CaseInsensitive(){
        try{
            List<String> categories = new ArrayList<>();
            Product expected = inventory.addProduct("Milk","Something to calm your baby face down.",ids,10, 5, categories, null);
    ...
    void testAddToCategory()throws Exception{
        ...
        Product p = inventory.addProduct("Milk","Something to calm your baby face down.",ids,10, 5, categories, null);
    ...
    void testRemoveProduct_Valid() throws Exception{
        ArrayList<String> categories = new ArrayList<>(Arrays.asList("Dairy","Momma's"));
        Product p = inventory.addProduct("Cocaine","Snowy white powder that gives you magical powers.",ids,10, 5, categories, null);
        ...
        assertDoesNotThrow(()->inventory.removeProduct(0, null)); //remove product here
    ...
    public void testAddSameProductConcurrently() throws Exception {
        ...
        for (int i = 0; i < 10; i++) {
            Callable<Product> task = () -> {
                ...
                return inventory.addProduct(name, description, prod_id,10, 5, categories, null);
            };
    ...
    public void testAddDifferentProductConcurrently() throws Exception {
        ...
        for (int i = 0; i < 10; i++) {
            ...
            Callable<Product> task = () -> {
                latch.countDown(); // decrement the count on each thread start
                latch.await(); // wait until all threads start
                return inventory.addProduct(name, description, ids,10, 5, categories, null);
            };
            executorService.submit(task);
        }
        executorService.shutdown();
...
//            expected.put(i,price);
            Callable<Product> task = () -> {
                latch.countDown(); // decrement the count on each thread start
                latch.await(); // wait until all threads start
                Product p = inventory.addProduct(name,description,ids,price, 5, categories, null);
...
//                inventory.setPrice(p.getID(),price);
    ...
    void testProductRatings_Solo_ValidInput() throws Exception{
        List<String> categories = new ArrayList<>();
        Product p = inventory.addProduct("Demon Blood","Used in satanic rituals, bitter taste.",ids,10, 5, categories, null);
    ...
    void testProductRatings_Solo_InvalidInput(){
        try {
            List<String> categories = new ArrayList<>();
            Product p = inventory.addProduct("Katana", "Used to slash innocent elderly and children.", ids, 10, 5, categories, null);
    ...
    void testProductRatings_Concurrent_InvalidInput() throws Exception {
        List<String> categories = new ArrayList<>();
        Product p = inventory.addProduct("Selfie Toaster","Burn your selfie onto your toast with the Selfie Toaster. Start your day with a smile and show off your best selfie to everyone... even your breakfast!", ids, 10, 5, categories, null);
        assertNotNull(p);
        ArrayList<Integer> ratings = new ArrayList<>(Arrays.asList(3,4,2,5,4,3,2,2,3,4,4));
        double expectedAvg = ratings.stream().mapToInt(Integer::intValue).average().orElse(0.0);
...
//    }
    ...
    void testFilterByProductRating() throws Exception{
        ...
        String ratingLow = "2";
        List<String> categories = new ArrayList<>();
        Product p1 = inventory.addProduct("prod1", "Description.", ids, 10, 5, categories, null);
        Product p2 = inventory.addProduct("prod2", "Nice Description.", ids, 10, 5, categories, null);
        Product p3 = inventory.addProduct("prod3", "Very Nice Description.", ids, 10, 5, categories, null);
        Product p4 = inventory.addProduct("prod4", "Not Very Nice Description.", ids, 10, 5, categories, null);
        int[][] productRatings = {
    ...
    void testFilterByPrice() throws Exception{
        ...
        List<String> categories = new ArrayList<>();
        Product p1 = inventory.addProduct("prod1", "Description.", ids, 2, 5, categories, null);
        Product p2 = inventory.addProduct("prod2", "Nice Description.", ids, 4, 5, categories, null);
        Product p3 = inventory.addProduct("prod3", "Very Nice Description.", ids, 6, 5, categories, null);
        Product p4 = inventory.addProduct("prod4", "Not Very Nice Description.", ids, 8, 5, categories, null);
        Product p5 = inventory.addProduct("prod5", "Maybe Not Very Nice Description.", ids, 10, 5, categories, null);
    ...
    void testFilterByCategories() throws Exception{
        ...
        Product p1 = inventory.addProduct("prod1", "Description.", ids, 10, 5, cat1, null);
        Product p2 = inventory.addProduct("prod2", "Nice Description.", ids, 10, 5, cat2, null);
        Product p3 = inventory.addProduct("prod3", "Very Nice Description.", ids, 10, 5, cat3, null);
        Product p4 = inventory.addProduct("prod4", "Not Very Nice Description.", ids, 10, 5, cat4, null);
        Product p5 = inventory.addProduct("prod5", "Maybe Not Very Nice Description.", ids, 10, 5, cat5, null);
    ...
    void testFilterProductsByKeywords() throws Exception {
        ...
        List<String> categories = new ArrayList<>();
        Product p1 = inventory.addProduct("prod1", "Our facial serum is the epitome of luxury - because who needs financial stability when you have flawless skin?.", ids, 10, 5, categories, null);
        Product p2 = inventory.addProduct("prod2", "Our memory foam mattress topper is the height of luxury - perfect for when you want to feel like you're sleeping on a cloud, instead of just dreaming about one..", ids, 10, 5, categories, null);
        Product p3 = inventory.addProduct("prod3", "This backpack will make you feel like a million bucks... or at least like you can afford the luxury toilet paper..", ids, 10, 5, categories, null);
        Product p4 = inventory.addProduct("prod4", "This backpack is tougher than your ex's heart, with more compartments than they had excuses..", ids, 10, 5, categories, null);
        Product p5 = inventory.addProduct("prod5", "No that's not 4kg of cocaine in my backpack, its just flour.", ids, 10, 5, categories, null);
