...
export const cartApi = 
{
    getCart: (params: GetCartParams) : Promise<ApiResponseListData<Basket>> =>
        apiErrorHandlerWrapper(noAuthApiClient.get('api/cart', {
            params: params
        })),

    patchCart: (params: PatchCartParams) : Promise<ApiResponse<string>> =>
        apiErrorHandlerWrapper(noAuthApiClient.patch('api/cart', params)),

    deleteCart: (params: DeleteCartParams) : Promise<ApiResponse<string>> =>
        apiErrorHandlerWrapper(noAuthApiClient.delete('api/cart', {
            params: params
        })),

    postBasket: (params: PostBasketParams) : Promise<ApiResponse<string>> =>
        apiErrorHandlerWrapper(noAuthApiClient.post('api/cart', params)),
